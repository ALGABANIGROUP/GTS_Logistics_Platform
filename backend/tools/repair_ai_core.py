# backend/tools/repair_ai_core.py
import os
import shutil
from datetime import datetime

def backup_and_repair_ai_core():
    """Backup and repair ai_core.py"""

    ai_core_path = "backend/core/ai_core.py"
    backup_path = f"backend/core/ai_core_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.py"

    print("🔧 Starting ai_core.py repair ...")
...
class OperationsManagerBot(AIBotBase):
    """Operations Manager AI Bot - FIXED VERSION"""
    ...
    async def process_message(self, message: str, context: Dict[str, Any] = None) -> str:
        ...
        try:
            if "workflow" in message.lower() or "operations" in message.lower():
                return "OperationsManagerBot: I can help optimize workflow and operational efficiency"
            elif "efficiency" in message.lower() or "improve" in message.lower():
                return "OperationsManagerBot: For better efficiency, I suggest reviewing resource allocation and process automation"
            else:
...
class FinanceBot(AIBotBase):
    ...
    async def process_message(self, message: str, context: Dict[str, Any] = None) -> str:
        ...
        try:
            if "report" in message.lower() or "financial" in message.lower():
                return "FinanceBot: I can generate detailed financial reports for expenses and revenues"
            elif "expense" in message.lower() or "budget" in message.lower():
                return "FinanceBot: I help track expenses and plan monthly budgets"
            elif "analysis" in message.lower():
                return "FinanceBot: I analyze financial data to provide strategic insights"
            else:
