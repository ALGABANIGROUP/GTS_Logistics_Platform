...
class Settings:
    ...
    SMTP_FROM: str = os.getenv("SMTP_FROM", "") or os.getenv("MAIL_FROM", "") or os.getenv("SMTP_USER", "")
    ...
    jwt_secret: str = os.getenv("JWT_SECRET") or os.getenv("JWT_SECRET_KEY") or os.getenv("GTS_JWT_SECRET") or SECRET_KEY
    ...
    def __post_init__(self) -> None:
        if self.APP_ENV == "production":
            ...
            if not self.ALLOWED_ORIGINS and not self.GTS_CORS_ORIGINS:
                raise ValueError("SECURITY WARNING: CORS origins not configured in production.")
