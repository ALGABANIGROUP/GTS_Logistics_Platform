...
# Config / DB init
# -----------------------------------------------------------------------------
VIZION_DATABASE_URL = os.getenv("VIZION_DATABASE_URL", "sqlite+aiosqlite:///./vizion.db")

engine = create_async_engine(VIZION_DATABASE_URL, echo=False, future=True)
SessionLocal = async_sessionmaker(engine, expire_on_commit=False, class_=AsyncSession)

def now_iso() -> str:
    # naive ISO string (like we used elsewhere)
...
class Task(Base):
    ...
    expected_minutes: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    ...
    due_ts: Mapped[Optional[str]] = mapped_column(String(32), nullable=True)
...
class Session(Base):
    ...
    end_ts: Mapped[Optional[str]] = mapped_column(String(32), nullable=True)
