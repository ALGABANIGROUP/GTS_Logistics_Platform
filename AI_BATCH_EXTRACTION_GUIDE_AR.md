# Ai Batch Extraction Guide Ar

        Updated: **2026-04-12**
        Documentation version: **2026.04.12-docs**

        ## Current Project Snapshot

- Documentation refresh date: **2026-04-12**
- Documentation version: **2026.04.12-docs**
- Frontend bot registry: **21 bots**, **21 active**
- New payment surfaces: **Payment Gateway Dashboard**, **AI Finance Bot** (SUDAPAY removed)
- Partner Manager status: **Active** and routed through `/ai-bots/partner-management`
- Operations additions: **Carriers** and **Shippers** workspaces with integrated frontend services
- Verified frontend build: **`npm run build` passed**
- Verified backend doc-related syntax checks: **`python -m py_compile backend/ai/bot_subscription_manager.py` passed**

## Key Active Routes

- `/ai-bots/hub`
- `/ai-bots/payment`
- `/ai-bots/finance`
- `/ai-bots/partner-management`
- `/ai-bots/carriers/*`
- `/ai-bots/shippers/*`
- `/payments/:invoiceId`
- `/payments/history`

## Latest Confirmed Changes Included In This Refresh

- Added the shared frontend bot registry and cleaned the hub display.
- Added and activated **Payment Gateway Dashboard** as a distinct bot entry.
- **Removed SUDAPAY** payment gateway from the project (deprecated and no longer supported).
- Activated **AI Partner Manager** across registries, dashboards, and routing.
- Added payment and finance summary helpers used by the dashboard.
- Added/update carrier and shipper frontend workspaces and service integrations.
- Updated bot alias handling so payment and partner routes resolve consistently.
- Updated all documentation to reflect SUDAPAY removal and current project state.

        ## Extraction Scope

Invoice and document extraction documentation is aligned with the current AI-assisted extraction direction and the invoice workflow now visible in finance and payment dashboards.

## Current Notes

- Invoice payment execution is tied to invoice identifiers
- Extraction outputs should feed finance and document workflows
- Production rollout still requires dataset validation and regression testing

        ## Standard Local Commands

```bash
# frontend
cd frontend
npm install
npm run dev
npm run build

# backend
python -m uvicorn backend.main:app --reload
```

        ## Maintainer Note

        This document was refreshed in English to match the latest confirmed project changes as of **2026-03-21**.
